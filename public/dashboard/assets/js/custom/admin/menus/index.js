...
const populateEditForm = (data) => {
    ...
    // ─── Icon ────────────────────────────────────────────────
    // نستخدم setIconPickerValue بس، مش هنكتب في iconInput.value مباشرة
    // عشان Select2 يتحدث صح مع الـ template والـ preview مع بعض
    setIconPickerValue('modal_edit_icon', 'modal_icon_preview', data.icon ?? '');
...
const initEdit = () => {
    ...
    document.addEventListener("hidden.bs.modal", (e) => {
        ...
        // ─── نرجّع الـ Select2 لحالته الافتراضية بعد إغلاق المودال ───
    ...
    document.querySelectorAll('.icon-picker-select').forEach((select) => {
        ...
        $(select).select2({
            ...
            templateResult: function (option) {
                ...
                // option.id دلوقتي هو الكلاس الكامل نفسه (زي "icon icon-activity" أو "ti-home")
...
const setIconPickerValue = (selectId, previewId, iconValue) => {
    ...
    if (iconValue) {
        ...
        // ─── fallback لو القيمة المخزنة قديمة (بدون base class مثلاً) ───
        if (!matchedOption) {
