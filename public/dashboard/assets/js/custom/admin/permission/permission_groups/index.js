 ...
 * ============================================================
 * 📁 FILE: index.js
 * 📌 MAIN FUNCTIONS: Permission Groups CRUD Operations
 * ============================================================
 */
...
const reloadTable = () => {
    try {
        window.LaravelDataTables["permission_groups_datatable"].ajax.reload(
            null,
            false,
        );
    } catch (e) {
        console.warn("reloadTable fallback:", e);
        $("#permission_groups_datatable").DataTable().ajax.reload(null, false);
    }
};
...
const showEditLoader = (show) => {
    ...
    el("editPermissionGroupForm").classList.toggle("d-none", show);
};

const populateEditForm = (data) => {
    el("editPermissionGroupId").value = data.id;
    el("edit_icon").value = data.icon ?? "";
    ...
    locales.forEach((input) => {
        ...
        if (match) {
            ...
            const field = match[2];
            const value = data.translations?.find(
                (t) => t.locale === locale
            )?.[field] ?? "";
            input.value = value;
        }
...
const initEdit = () => {
    document.addEventListener("click", async (e) => {
        ...
        getModal("editPermissionGroupModal").show();
        showEditLoader(true);
        ...
        try {
            ...
            if (data?.success) {
                ...
                hideModal("editPermissionGroupModal");
                Alert.error(window.translations.error);
            }
        } catch (err) {
            console.error(err);
    ...
    document.addEventListener("click", async (e) => {
        if (!e.target.closest("#saveEditPermissionGroup")) return;
        const saveBtn = el("saveEditPermissionGroup");
        const id = el("editPermissionGroupId").value;
        const form = el("editPermissionGroupForm");
        if (!id) {
            Alert.error(window.translations.error);
            return;
        }
    ...
    document.addEventListener("hidden.bs.modal", (e) => {
        if (e.target.id !== "editPermissionGroupModal") return;
        el("editPermissionGroupForm")?.reset();
        showEditLoader(true);
    });
};
...
const initDelete = () => {
    document.addEventListener("click", (e) => {
        ...
        if (!btn) return;
        el("deletePermissionGroupId").value = btn.dataset.id;
        getModal("deletePermissionGroupModal").show();
    });

    document.addEventListener("click", async (e) => {
        if (!e.target.closest("#confirmDeletePermissionGroup")) return;

        const confirmBtn = el("confirmDeletePermissionGroup");
        const id = el("deletePermissionGroupId").value;
        if (!id) return;
        ...
        try {
            ...
            if (data.success) {
                Alert.success(data.message);
                hideModal("deletePermissionGroupModal");
                reloadTable();
    ...
    document.addEventListener("hidden.bs.modal", (e) => {
        if (e.target.id !== "deletePermissionGroupModal") return;
        el("deletePermissionGroupId").value = "";
    });
};
...
const initRestore = () => {
    document.addEventListener("click", async (e) => {
        ...
        // ✅ استخدم data-route مباشرة من الزر
        ...
        showConfirmModal(
            ...
            window.translations.restore_confirm ||
                "هل أنت متأكد من استعادة هذه المجموعة؟",
            window.translations.restore || "استعادة",
            async (confirmBtn) => {
                try {
...
// ─── تعريف PermissionGroups كـ object عالمي ──────────────────────────────
window.PermissionGroups = {
    init: init,
